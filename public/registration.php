        ...
        <div class="registration">
            ...
            <form action="methods/reg.php" method="post" class="registration__form">
                ...
                <div class="form-group">
                    ...
                    <input class="form__input" type="text" name="name" id="name" placeholder="Артем"
                    value="<?= $_SESSION['test_user']->name ?>" required />
                </div>
                <div class="form-group">
                    <label class="form__label" for="email">Email:</label>
                    <input class="form__input" type="email" name="email" id="email" placeholder="Email address"
                    value="<?= $_SESSION['test_user']->email ?>" required />
                </div>
                <div class=" form-group">
                    <label class="form__label" for="password">Пароль:</label>
                    <input class="form__input" type="password" name="password" id="password" placeholder="Password"
                    value="<?= $_SESSION['test_user']->password ?>" required />
                </div>
                <div class="form-group">
                    <label class="form__label" for="password_retard">Повтор пароля:</label>
                    <input class="form__input" type="password" name="passwordRetard" id="passwordRetard"
                    placeholder="Retard password" value="<?= $_SESSION['test_user']->passwordRetard ?>" required />
                </div>
                <div class="form-group">
                    <label class="form__label form__label_order2"
                    for="radioButton">Согласие на обработку персональных данных</label>
                    <input type="radio" onMouseDown="this.isChecked=this.checked;"
                    onClick="this.checked=!this.isChecked;"
                    id="radioButton" name="personalData" class="form__label_order1">
                </div>
